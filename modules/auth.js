...
export function initAuth(firebaseApp, onUserChanged) {
  ...
  // GitHub provider — works once enabled in Firebase Console
  try {
    githubProvider = new firebase.auth.GithubAuthProvider();
  } catch (e) {
    console.warn('GitHub auth provider unavailable:', e.message);
  }
...
function _renderModalContent() {
  ...
  box.innerHTML = `
    ...
    <div class="am-providers">
      <button class="am-provider-btn am-github" id="amGithubBtn">
  ...
  box.querySelector('#amGithubBtn').addEventListener('click', () => {
    if (!githubProvider) {
      _showError('GitHub sign-in requires additional setup. Please use Google or Email.');
      return;
    }
    _signInWithProvider(githubProvider, 'GitHub');
  });
